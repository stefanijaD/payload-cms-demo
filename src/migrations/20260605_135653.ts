...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TABLE "instructions" (
  	...
  	"instruction_type" "enum_instructions_instruction_type" NOT NULL,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  CREATE TABLE "shared_steps_locales" (
  ...
  ALTER TABLE "shared_steps_locales" ADD CONSTRAINT "shared_steps_locales_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."shared_steps"("id") ON DELETE cascade ON UPDATE no action;
  ...
  ALTER TABLE "payload_locked_documents_rels" ADD CONSTRAINT "payload_locked_documents_rels_shared_steps_fk" FOREIGN KEY ("shared_steps_id") REFERENCES "public"."shared_steps"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE INDEX "shared_steps_updated_at_idx" ON "shared_steps" USING btree ("updated_at");
  CREATE INDEX "shared_steps_created_at_idx" ON "shared_steps" USING btree ("created_at");
  CREATE UNIQUE INDEX "shared_steps_locales_locale_parent_id_unique" ON "shared_steps_locales" USING btree ("_locale","_parent_id");
  ...
  CREATE INDEX "payload_locked_documents_rels_shared_steps_id_idx" ON "payload_locked_documents_rels" USING btree ("shared_steps_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "shared_steps" CASCADE;
  DROP TABLE "shared_steps_locales" CASCADE;
